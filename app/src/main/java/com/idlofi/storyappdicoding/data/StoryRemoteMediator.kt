...
import com.idlofi.storyappdicoding.database.RemoteKeys
...
class StoryRemoteMediator(
    ...
    context: Context
) : RemoteMediator<Int, StoryResponItem>() {
    ...
    override suspend fun load(
        loadType: LoadType,
        state: PagingState<Int, StoryResponItem>
    ): MediatorResult {
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, StoryResponItem>): RemoteKeys? {
        return state.pages.lastOrNull { it.data.isNotEmpty() }?.data?.lastOrNull()?.let { data ->
            database.remoteKeysDao().getRemoteKeysId(data.id)
        }
    }

    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, StoryResponItem>): RemoteKeys? {
        return state.pages.firstOrNull { it.data.isNotEmpty() }?.data?.firstOrNull()?.let { data ->
            database.remoteKeysDao().getRemoteKeysId(data.id)
        }
    }

    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, StoryResponItem>): RemoteKeys? {
